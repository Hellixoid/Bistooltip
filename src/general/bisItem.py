...
class BisItem:
    ...
    def add_bis_statement(self, spec_id, slot_name, phase_name, ordinal):
        spec = self.specs.get(spec_id)
        if spec is None:
            spec = Spec(spec_id)
            self.specs[spec_id] = spec
        spec.add_bis_statement(slot_name, phase_name, ordinal)
...
class Spec:
    ...
    def add_bis_statement(self, slot_name, phase_name, ordinal):
        slot = self.slots.get(slot_name)
        if slot is None:
            slot = Slot(slot_name)
            self.slots[slot_name] = slot
        slot.add_bis_statement(phase_name, ordinal)
...
class Slot:
    ...
    def add_bis_statement(self, phase_name, ordinal):
    ...
    def __str__(self):
        return f'{{ ["name"] = "{self.name}", ["ranks"] = "{self.phase_bis_str()}" }}'
